let minimo = prompt("Ingresa el valor mínimo a adivinar");
...
for (let i = 0; i < 3; i++) {
  ...
  if (adivinarNumero == numero) {
    alert(`Adivinaste el número y es ${adivinarNumero}`);
    break;
  } else {
    alert("Prueba nuevamente");
    alert(`Te quedan ${2 - i} intentos`);
    /*alert(`El número a adivinar es ${adivinarNumero}`);*/
  }
}
...
Debes crear un pequeño programa que comience con un valor específico predefinido entre 0 y 10 para el número que vas a adivinar 
(por ejemplo, el 7).
A continuación, el programa te preguntará cuál es el valor que deseas adivinar y, si aciertas, te felicitará. Si te equivocas, 
te dará 2 intentos más.
